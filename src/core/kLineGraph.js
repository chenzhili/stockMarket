...
export function initkLineGraph (QL, data) {
  ...
  let startI = data.data.length - calcConfig.kLineGraph.initShowN; let endI = data.data.length
...
export function kLineGraphPaint (data) {
  ...
  let RLMin = null; let RLMax = null; let RYFactor = null; let // 这是 k 线的 走势图
    RDMin = null; let RDMax = null; let DDFactor = null; const // 这是成交量 的 图
  ...
  let showItem = null
  let ax // 实际的 绘制的 中心 坐标值
  let colorStroke = '#000' // 描边的 颜色
  let colorFill = '#000' // rect 填充的颜色
  const MaArr = calcConfig.kLineGraph.MAConfig.map(i => false) // 存储 当前 是否 可以 绘制 均线
  ...
  for (let i = 0, len = showData.length; i < len; i++) {
    ...
    const asend = showItem.close > showItem.open
