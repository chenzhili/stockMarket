...
function calRangeValue (targetValue, closePrice) {
  ...
  let max = null; let min = null; const dValue = null
  const curMax = temArrByCur[len - 1]
  const curMin = temArrByCur[0]
  const avMax = tempArrByav[len - 1]
  const avMin = tempArrByav[0]

  /* 这里的 逻辑 需要重写
        对于 上下界限 为了有 容错性，把 最大值 和 最小值，以及 默认 浮动 比例 ，来做这块；
    */
  const floatRate = 0.1 // 以 内陆 股市 为基准；
  const floatUp = closePrice * (1 + floatRate); const floatDown = closePrice * (1 - floatRate)
  ...
  const dUpValue = max - closePrice; const dDownValue = closePrice - min
...
function calActuallyHeight (QL, config) {
  ...
  return config.sort.reduce((prev, next, index) => {
    ...
    const regExp = new RegExp(`^${allGraph.text}`, 'ig')
    prev[name] = prev[name] || {}
...
function calTimeValuePos ({ min, max, prevClose, totalHeight, baseHeight, n, decimal = 100 }) {
  ...
  const actuallyValue = [] // 现价
  const valueYPos = [] // 实际 纵坐标

  const upFactor = (max - prevClose) / halfN; const downFactor = (prevClose - min) / halfN

  let i = 0; let actuallyFactor = null; let curAct = null; let curInc = null
  while (i < n) {
...
export function paintTimeSharingDiagram (data) {
  ...
  let LMin = null; let LMax = null; const LYFactor = null; let LYUpFactor = null; let LYDownFator = null; let // 这是 line图的,需要对于 基准线上下进行区分

    DMin = null; let DMax = null; let DYFactor = null // deal 图
  const xFactor = QL._DOMWidth / 240 // 这里的 240 是指 分成了 4段 ，每段 分为 60min
  ...
  let curX = null; let actuallyItem
  ...
  const tempLen = chartData.length; const range = 60; const maxRange = 240
  let n = Math.ceil(tempLen / range); let tempSX = 0
