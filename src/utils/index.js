...
export function calValuePos ({ min, max, factorMaxInc, totalHeight, baseHeight, n, decimal = 100 }) {
  ...
  const valueIncrement = (max - min) / (n - 1); const yPosIncrement = totalHeight / (n - 1)
  const config = {
    actuallyValue: [],
    valueYPos: []
  }
  ...
  return new Array(n).fill(1).reduce((prev, next, index) => {
    ...
    const tempPos = parseInt((baseHeight + index * yPosIncrement) * 100) / 100
    const tempF = f ? parseInt((factorMaxInc - f * index) * 100) / 100 : null
